...
export class NotificationManager {
  ...
  async shouldRestart(config: vscode.WorkspaceConfiguration): Promise<boolean> {
    const autoRestart = config.get("autoRestart", true);
    if (autoRestart) return true;

    const choice = await vscode.window.showInformationMessage(
      "Configuration file changed. Restart development server?",
      "Restart Now",
      "Remind Later",
      "Ignore"
    );
  ...
  private scheduleReminder() {
    setTimeout(() => {
      vscode.window.showWarningMessage(
        "⚠️ Env Watcher: Server restart still pending. Configuration changes may not be active.",
        "Restart Now",
        "Dismiss"
      ).then(choice => {
        if (choice === "Restart Now") {
  ...
  showEnvChangeReminder(onRestart: () => void) {
    const notification = vscode.window.showWarningMessage(
      "Environment file changed. Consider restarting your development server.",
      "Restart Now",
      "Dismiss"
    );
  ...
  showLightweightReminder() {
    const notification = vscode.window.showInformationMessage(
      "💡 Environment file changed. Consider restarting your server."
    );
